...
static void dbg_mark(unsigned short x, char c) {
    volatile unsigned short* vram = (volatile unsigned short*)0xB8000;
    vram[x] = (unsigned short)c | 0x0F00;
}
/*
KERNEL PUD:
...
void k_init_kernel_pages(int kernel_size) {
    ...
    temp_PUD = (pud_t*)((u64)temp_PUD + kernel_size_aligned);
    dbg_mark(3, 'B');
    ...
    link_ptable(kernel_PGD, kernel_PUD, 511, PAGE_PRESENT | PAGE_WRITEABLE| PAGE_GLOBAL);
    dbg_mark(5, 'D');
    ...
    if (map_memory_1G(kernel_PGD, 0, PAGE_PRESENT | PAGE_WRITEABLE| PAGE_GLOBAL, 511, 510) != 0) {
        dbg_mark(6, 'E');
        for(;;){ __asm__ __volatile__("hlt"); }
    }
    dbg_mark(6, 'F');
    link_ptable(kernel_PUD, kernel_stack_PMD, 509, PAGE_PRESENT | PAGE_WRITEABLE| PAGE_GLOBAL);
    dbg_mark(7, 'G');
    //build heap map
    for(int i=0;i<32;i++){
        if (map_memory_2m(kernel_PGD, 2*MB+kernel_size_aligned+2*MB+i*2*MB, PAGE_PRESENT | PAGE_WRITEABLE| PAGE_GLOBAL, 511, 509, 511-i) != 0) {
            dbg_mark(8, 'H');
            for(;;){ __asm__ __volatile__("hlt"); }
        }
    }
    ...
    dbg_mark(10, 'L');
    link_ptable(kernel_PGD, phys_PUD0, 509, PAGE_PRESENT | PAGE_WRITEABLE| PAGE_GLOBAL);
    link_ptable(kernel_PGD, phys_PUD1, 510, PAGE_PRESENT | PAGE_WRITEABLE| PAGE_GLOBAL);
    dbg_mark(11, 'M');
    for(int i=0;i<get_phys_size()/GB;i++){
        if (map_memory_1G(kernel_PGD, i*GB, PAGE_PRESENT | PAGE_WRITEABLE| PAGE_GLOBAL, 509+i/512, i) != 0) {
            dbg_mark(12, 'N');
            for(;;){ __asm__ __volatile__("hlt"); }
        }
    }
    ...
    temp_PUD = (pud_t*)((u64)temp_PUD + PHYSICAL_VIRT);
}
